...
bool IsProcessRunning(const std::string& processName) {
	...
	if (pos != std::string::npos && name.substr(pos) == ".exe") {
	...
	PROCESSENTRY32W entry;  // 使用宽字符版本
	entry.dwSize = sizeof(PROCESSENTRY32W);
	
	if (Process32FirstW(snapshot, &entry)) {  // 使用W版本
		do {
			// 直接比较宽字符串
			...
			MultiByteToWideChar(CP_ACP, 0, name.c_str(), -1, &wName[0], len);
			wName.pop_back(); // 移除结尾的null字符
			
			// 去除 .exe 后缀（如果有）
			size_t dotPos = wExeName.find_last_of(L".");
			if (dotPos != std::wstring::npos && wExeName.substr(dotPos) == L".exe") {
				wExeName = wExeName.substr(0, dotPos);
			}
			
			// 不区分大小写比较
			if (_wcsicmp(wExeName.c_str(), wName.c_str()) == 0) {
...
int main() {
	...
	while (1) {
		qdd:
		system("cls");
		cout << "当前位置：" << GetExeDirectory() << endl;
		cout << "当前程序位置：" << GetExePath() << endl;
		cout << "=======================================================" << endl;
		...
		cout << "|   |/    \\|  |__|                     \\/   /      \\  |            " << endl;
		cout << "|                                                     |" << endl;
		cout << "|          Windows     Start     Application          |" << endl;
		cout << "=======================================================" << endl;
		cout << "按q键退出" << endl;
		cout << "exe $ ";
		cin >> aa;
		...
		string command = "start " + aa;
		system(command.c_str());
		Sleep(1200);
		
		if (IsProcessRunning(aa)) {
			cout << "应用已经正在运行" << endl;
			system("pause");
			goto qdd;
		} else {
			cout << "应用未运行，可能是没加exe后缀，也可能是当前目录下没有这个应用。";
			system("pause");
		}
	}
